...
define([
    ...
    return angular.module('xwWeb')
        .registerController('recommendationController', ['$scope', 'CONFIG', 'OpportunityService','$uibModal','$stateParams', 'authService', 'errorService', 'recommendService', 'shopService', 'requirementService', '$q', 'districtService', 'industryService', 'cityService', 'resourceService', 'paginationService', '$window', recommendationController]);

    function recommendationController($scope, CONFIG,OpportunityService,$uibModal, $stateParams, authService, errorService, recommendService, shopService, requirementService, $q, districtService, industryService, cityService, resourceService, paginationService, $window) {
        var vm = this;
        ...
        function matchTransfer() {
            ...
            resourceService.matchTransfer(sessionId, parseInt(serviceId), params, vm.currentPage - 1, vm.pageSize).then(function (result) {
                ...
                vm.complete = true;
                for (var i in vm.recommendation) {
                    vm.recommendation[i]['showDetail'] = false;
                }
        ...
        function showDetail(index){
            vm.recommendation[index]['showDetail'] = true;
            if (vm.recommendation[index]['detail']) return;
            OpportunityService.getInfo(sessionId, vm.recommendation[index]['id']).then(function(data) {
                vm.recommendation[index]['detail'] = data;
            }, function(error) {

            });
        }
        function hideDetail(index) {
            vm.recommendation[index]['showDetail'] = false;
        }
